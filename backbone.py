...
class LabelEmbedder(nn.Module):
    ...
    def forward(self, labels):
        return self.embedding(labels)  # y_emb
...
class MFDiT(nn.Module):
    ...
    def unpatchify(self, x):
        ...
        return x.reshape(shape=(x.shape[0], c, h * p, h * p))  # imgs

    def _time_condition(self, t, r):
        embeddings = []
        if self.condition == ConditionType.T_R:
            embeddings.extend((self.t_embedder(t), self.r_embedder(r)))
        elif self.condition == ConditionType.T_DELTA_T:
            embeddings.extend((self.t_embedder(t), self.r_embedder(t - r)))
    ...
    def forward(self, x, t, r, y=None):
        ...
        # r = self.r_embedder(r)
        # t = t + r
        t = self._time_condition(t, r)  # (N, D)
